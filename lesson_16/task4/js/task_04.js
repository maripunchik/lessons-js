...
if (confirm("Почати тестування?")) {
  ...
  class TBankomat {
    ...
    withdraw(amount) {
      ...
      const plan = []; // масив з посиланнями на об'єкти Bill та кількістю для зменшення

      // Алгоритм для вибору купюр
       for (let bill of this.banknotes) {
         const countNeeded = Math.floor(remaining / bill.Denomination);
         const countToGive = Math.min(countNeeded, bill.Amount);

         if (countToGive > 0) {
           plan.push({ bill, count: countToGive });
       ...
       // Якщо залишок суми не 0, значить не можна видати точну суму
       if (remaining !== 0) {
         throw new Error("Неможливо видати точну суму наявними купюрами");
       }
      

      // План видачі готовий, оновлюємо кількість купюр
      let withdrawSummary = "";
      for (let { bill, count } of plan) {
        bill.Amount -= count;
        withdrawSummary += `Видано ${count} купюр(и) по ${bill.Denomination} грн\n`;
      }
