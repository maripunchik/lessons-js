...
const cssObj = {
  translatorWrapper: "translator-wrapper",
...
class WordCard {
  ...
  render(containerSelector) {
    ...
    divElement.onclick = this.onWordSelect.bind(this); // onclick прив1язали метод .bind(this)
	 ...
	 divElement.className= this.cssObj.base
...
class WordsList {
  ...
  render(containerSelector) {
    ...
    containerEl.className = `${this.cssObj.container} ${this.lang}`;

    let wordData;
    do {
      wordData = this.takeOutRandomWord(); // випадково вибрали слово
      if (wordData) {
        // перевіряємо умову, чи слово згенерувалось
...
class Translator {
  ...
  render(containerSelector) {
    const containerEl = document.createElement("div");
    containerEl.className = this.cssObj.translatorWrapper;
