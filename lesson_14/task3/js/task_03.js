...
if (confirm("Почати тестування?")) {
  ...
  const totalPrice = dataList.reduce(
    (prevTotalPrice, el) => prevTotalPrice + el.price,
    0
  );
  // ---- або
//   let totalPrice = 0;
...
//   }
  ...
  // 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
  const reducedCount = dataList.reduce((count, el) => {
    if (el.old_price > 0 && el.old_price > el.price) {
      return count + 1;
    }
    return count;
  }, 0);
  ...
  console.log(`Кількість товарів, у яких ціна зменшилась : ${reducedCount}`);
  // 3) Товари, які доступні (sell_status:"available")
  const availableGoods = dataList.filter(
    (el) => el.sell_status === "available"
  );
  console.log(`Кількість доступних товарів: ${availableGoods.length}`);
  // 4) сформувати новий список об”єктів тільки доступних  для продажу товарів, які міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну (old_price), та ціну у доларах (usd_price)
  const listOfGoodsAvailableForSale = availableGoods.map((el) => ({
    id: el.id,
    price: el.price,
    old_price: el.old_price,
    usd_price: el.usd_price,
  }));

  console.table(listOfGoodsAvailableForSale);
}
